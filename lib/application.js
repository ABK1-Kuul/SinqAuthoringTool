...
Origin.prototype.createServer = function (options, cb) {
  ...
  */
  if (options.minimal) {
    ...
    // Serve favicon only if it exists; otherwise skip to avoid throw
    const faviconPath = path.join(require('./configuration').serverRoot, 'frontend', 'src', 'favicon.ico');
    ...
    // Serve build assets and libraries so CSS/JS load correctly in minimal mode
  ...
  */
  ...
  require('./database').getDatabase(function (err, db) {
    ...
    // Ensure db and connection are present before proceeding
    if (!db || !db.conn || !db.conn.connectionUri) {
      return cb(new Error('Database connection not available'));
    }
    server.use(compression());
    server.use(bodyParser.urlencoded({ extended: true }));
    server.use(methodOverride());
    ...
    server.use(bodyParser.urlencoded({ limit: '50mb', extended: true }));
    // Session store setup; guard against invalid store to prevent app.use errors
    let sessionStore;
    const dbType = configuration.getConfig('dbType');
    if (dbType === 'sqlite') {
      const SQLiteStore = require('connect-sqlite3')(session);
      sessionStore = new SQLiteStore({
        db: 'sessions.db',
        dir: path.join(configuration.getConfig('dataRoot') || 'data', 'db')
      });
      app.sessionStore = sessionStore;
    } else if (dbType === 'pg') {
      const PgSession = require('connect-pg-simple')(session);
      sessionStore = new PgSession({
        conString: db.conn.connectionUri,
        tableName: 'session'
      });
    } else {
    ...
    // Ensure session secret exists - required for session middleware
    // Priority: SESSION_SECRET env > .session-key file > generate & persist (never from config)
    const configPath = app.options && app.options.configFile;
    const baseDir = configPath && path.isAbsolute(configPath)
      ? path.dirname(configPath)
      : path.join(require('./configuration').serverRoot, 'conf');
    const secret = sessionSecret.getSessionSecret(baseDir);
    ...
    server.use(session({
      ...
      cookie: {
        signed: true,
        secureProxy: app.configuration && app.configuration.getConfig('isProduction')
      },
...
Origin.prototype.startServer = function (options) {
  ...
  console.log('[app] serverRoot:', app.configuration && app.configuration.serverRoot);
  console.log('[app] configFile:', app.options && app.options.configFile);
  console.log('[app] dbName:', app.configuration && app.configuration.getConfig('dbName'));
  console.log('[app] minimal:', serverOptions.minimal);
    app.createServer(serverOptions, function (error, server) {
      if (error) {
        logger.log('fatal', 'error creating server', error);
        return process.exit(1);
      }
      // use port from options first, then config, then default
      const optionsPort = options && options.serverPort && Number(options.serverPort);
      const cfgPort = app.configuration && app.configuration.getConfig('serverPort');
      var port = optionsPort || (cfgPort && Number(cfgPort)) || DEFAULT_SERVER_PORT;
      ...
      app._httpServer = httpServer.listen(port, function() {
        ...
        var writeRebuildFile = function(courseFolder, callback) {
          ...
          if (!fs.existsSync(buildFolder)) return callback(null);
...
async function runMigrations(app) {
  if (configuration.getConfig('dbType') === 'sqlite' || configuration.getConfig('dbType') === 'pg') {
    return;
  }
  try {
    ...
    if(!collections.find(c => c.collectionName === 'changelog')) {
      ...
      await installHelpers.runMigrations({ app });
