...
Origin.prototype.createServer = function (options, cb) {
  ...
  */
  ...
  require('./database').getDatabase(function (err, db) {
    ...
    let sessionSecret = app.configuration && app.configuration.getConfig('sessionSecret');
    if (!sessionSecret) {
      // Generate a secure random secret if not configured
      const crypto = require('crypto');
      sessionSecret = crypto.randomBytes(64).toString('hex');
      if (app.configuration) {
        app.configuration.setConfig('sessionSecret', sessionSecret);
      }
      logger.log('warn', 'Session secret not found in config, generated new one');
    }
