        ...
        db.retrieve('component', {_courseId: courseId}, {operators: {distinct: '_component'}}, function (err, components) {
          db.retrieve('componenttype', {}, function(err, componentTypes) {
            ...
            async.map(components, function(component, callback) {
              var compName = (typeof component === 'object' && component !== null) 
                ? (component._component || component.component) 
                : component;
              var componentType = _.find(componentTypes, function(ct) {
                return ct.component === compName;
              });
              if (!componentType) {
                return callback(null, null); // Skip missing component types to prevent crash, or map gracefully
              }
              return callback(null, { name: componentType.name });
            }, function(err, uniqueComponentList) {
              uniqueComponentList = (uniqueComponentList || []).filter(Boolean);
