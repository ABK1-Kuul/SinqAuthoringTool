...
* Requires opts.app (or app from application module) with bowermanager.
* Skips plugin update if app/bowermanager is unavailable (e.g. CLI context).
*/
function updateFrameworkPlugins(opts, callback) {
  ...
  if (!app || !app.bowermanager) {
    try {
      app = require(path.resolve(__dirname, 'application'))();
    } catch (e) {
      app = null;
    }
  }
  if (!app || !app.bowermanager) {
    warn('Skipping framework plugin update: app/bowermanager not available');
    return callback();
  }
