...
var path = require('path'),
    ...
    // Require the backend application directly to avoid loading the Electron main process in tests
    origin = require('./application');
...
ContentPlugin.prototype.getSchemaPath = function () {
  ...
  if (COURSE_COLLECTIONS.includes(type)) {
    const frameworkSchemaPath = path.join(
    ...
    // Fallback to bundled plugin schema if framework schema is not present (e.g. in tests)
    if (fs.existsSync(frameworkSchemaPath)) {
      return frameworkSchemaPath;
    }
  }

  return path.join(configuration.serverRoot, 'plugins', 'content', type, 'model.schema');
};
...
ContentManager.prototype.setupRoutes = function () {
  ...
  rest.get('/content/schema', function(req, res, next) {
    ...
    processPlugins(memo, function(err) {
      ...
      filterSchemas(memo, function(err, schemas) {
        ...
        if(coreExtensionsGlobals && schemas.course && schemas.course._globals && schemas.course._globals.properties && schemas.course._globals.properties._extensions) {
        ...
        if(schemas.course && schemas.course._globals && schemas.course._globals.properties && schemas.course._globals.properties._components) {
    ...
    function filterSchemas(memo, callback) {
      ...
      async.each(Object.keys(memo.schemas), function(key, callback) {
        ...
        async.each(memo.pluginTypes, function(plugin, cb) {
          ...
          if((schema._doc || schema).component && schema.properties[plugin.settingsProperty]) {
          ...
          db.retrieve('componenttype', {}, function (err, componentTypes) {
            ...
            async.each(results, function(component, callback) {
              ...
              if (component._doc) {
                ...
                component._componentTypeDisplayName = (match) ? match.displayName : '';
              }
  ...
  // apply preset to a course configuration
  rest.post('/content/preset/apply', function (req, res, next) {
    var courseId = req.body.courseId;
    var presetKey = req.body.presetKey;
    if (!courseId || !presetKey) {
      res.statusCode = 400;
      return res.json({ success: false, message: 'courseId and presetKey are required' });
    }
    ...
    var presets = {
      ...
      gamified: {
        _globals: {
          _accessibility: {
            accessibilityToggleTextOn: "Accessibility On",
            accessibilityToggleTextOff: "Accessibility Off",
            skipNavigationText: "Skip Navigation",
            _accessibilityInstructions: {
              touch: "Swipe to navigate.",
              notouch: "Use keyboard arrow keys to navigate.",
              ipad: "Swipe or use keyboard arrow keys."
            }
          }
        },
        _start: {
    ...
    if (!presetData) {
      res.statusCode = 400;
      return res.json({ success: false, message: 'Invalid preset key' });
    }

    // Retrieve existing course first
    that.retrieve('course', { _id: courseId }, {}, function (error, courses) {
      ...
      deepMerge(course, presetData);

      // Save updated course
      that.update('course', { _id: courseId }, course, function (err, result) {
        if (err) {
          res.statusCode = 500;
      ...
      async.series([
        ...
        function(callback) {
          db.retrieve(referenceType, { _id: id }, function (error, results) {
            ...
            var item = results[0]._doc || results[0];
        ...
        function(callback) {
          ...
          db.retrieve('article', { _parentId: contentObjects[0]._id }, function(error, results) {
            ...
            async.eachSeries(results, function(result, cb) {
              articles.push(result._doc || result);
              cb();
            }, callback);
          });
        },
          ...
          async.eachSeries(articles, function(article, cb) {
            db.retrieve('block', { _parentId: article._id }, function(error, results) {
              ...
              async.eachSeries(results, function(result, cb2) {
                blocks.push(result._doc || result);
                cb2();
              }, cb);
            });
          }, callback);
        ...
        function(callback) {
          ...
          async.eachSeries(blocks, function(block, cb) {
            db.retrieve('component', { _parentId: block._id }, function(error, results) {
              ...
              async.eachSeries(results, function(result, cb2) {
                components.push(result._doc || result);
                cb2();
              }, cb);
            });
          }, callback);
  ...
  rest.post('/content/clipboard/paste', function(req, res, next) {
    ...
    database.getDatabase(function (error, db) {
      ...
      db.retrieve('clipboard', {id: id, createdBy: user._id}, function(error, results) {
        ...
        clipboard = results[0]._doc || results[0];
        ...
        async.series([
          function(callback) {
            ...
            db.retrieve(parentRelationship[clipboard.referenceType], { _id: parentId }, function(error, results) {
              ...
              if (results.length === 1) {
                parentObject = results[0]._doc || results[0];
                return callback();
              }
