...
exports = module.exports = {
  ...
  policyChecker: function () {
    return (req, res, next) => {
      ...
      let user = app.usermanager.getCurrentUser();
      if (!user && req.session && req.session.passport && req.session.passport.user) {
        try {
          user = JSON.parse(JSON.stringify(req.session.passport.user));
        } catch (e) {}
      }
