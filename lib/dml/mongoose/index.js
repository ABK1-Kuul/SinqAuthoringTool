...
MongooseDB.prototype.connect = function(db) {
  ...
  return mongoose.createConnection(connectionString, options).then(function(conn) {
    ...
    this.conn.once('error', function(err) {
      logger.log('error', 'Database Connection failed, please check your database:', err);
    });
    ...
    logger.log('error', 'Failed to create MongoDB connection:', err);
    logger.log('error', 'Connection string:', connectionString);
    throw err;
  });
};
