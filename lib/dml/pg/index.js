...
function getPopulatedKeys(populate) {
  if (!populate) return [];
  if (typeof populate === 'string') {
    return populate.split(/\s+/).filter(Boolean);
  }
  if (Array.isArray(populate)) {
    return populate.map(p => typeof p === 'string' ? p : p.path).filter(Boolean);
  }
...
function projectDoc(doc, fields, populate) {
  ...
  if (typeof fields === 'string') {
    ...
    fieldsObj = {};
    fields.forEach(f => { fieldsObj[f] = 1; });
  }

  if (typeof fieldsObj !== 'object') return doc;

  const keys = Object.keys(fieldsObj);
  if (keys.length === 0) return doc;

  const inclusive = keys.some(k => k !== '_id' && (fieldsObj[k] === 1 || fieldsObj[k] === true));

  const projected = {};
  if (inclusive) {
    keys.forEach(k => {
      if ((fieldsObj[k] === 1 || fieldsObj[k] === true) && doc[k] !== undefined) {
        projected[k] = doc[k];
      }
    });
    const popKeys = getPopulatedKeys(populate);
    popKeys.forEach(k => {
      if (doc[k] !== undefined) {
        projected[k] = doc[k];
      }
    });
    if (fieldsObj._id !== 0 && fieldsObj._id !== false && doc._id !== undefined) {
      projected._id = doc._id;
    }
  } else {
    Object.keys(doc).forEach(k => {
      if (fieldsObj[k] !== 0 && fieldsObj[k] !== false) {
        projected[k] = doc[k];
      }
    });
  }
...
function wrapDoc(data, modelName, dbInstance) {
  ...
  if (schema && schema.properties) {
    Object.keys(schema.properties).forEach(key => {
      ...
      if (doc[key] === undefined && prop && prop.type === 'array') {
        doc[key] = [];
      }
      if (doc[key] !== undefined && doc[key] !== null && prop && (prop.type === Date || prop.type === 'date')) {
...
PostgresDB.prototype.addModel = function(modelName, schema, cb) {
  this.addSchema(modelName, schema);
  if (cb) cb(null, schema);
};
...
async function populateDocs(dbInstance, modelName, docs, populateOpt) {
  ...
  for (const p of paths) {
    ...
    const prop = schema.properties ? schema.properties[field] : schema[field];
    if (prop) {
      if (prop.ref) {
...
PostgresDB.prototype.retrieve = function(objectType, search, options, callback) {
  ...
  this.client.query(sql, params, (err, res) => {
    ...
    populateDocs(this, table, results, options.populate).then((populated) => {
      let finalResults = populated.map(doc => wrapDoc(doc, table, this));
      if (options.fields) {
        finalResults = finalResults.map(doc => {
          const projected = projectDoc(doc, options.fields, options.populate);
          Object.defineProperty(projected, 'toObject', {
            value: function() { return this; },
            enumerable: false,
            writable: true,
            configurable: true
          ...
          Object.defineProperty(projected, 'save', {
            value: function(cb) {
              self.update(table, { _id: this._id }, this, function(err, result) {
                if (cb) cb(err, result);
              });
            },
            enumerable: false,
