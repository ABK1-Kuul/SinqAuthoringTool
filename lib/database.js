...
function createDatabaseInstance (type, dbConfig, next) {
  ...
  dbInstance.connect(dbConfig).then(function() {
    ...
    dbInstance.loadSchemas(path.join(getSchemaPath(), 'system'), function (error) {
      if (error) {
        logger.log('error', 'Failed to load schemas:', error);
      ...
      processDatabaseHooks('create', dbInstance, function(err) {
        if (err) {
          ...
          logger.log('error', 'Database hook processing failed:', err);
          delete _databases[type][dbConfig.dbName];
          return next(err);
        }
    ...
    logger.log('error', 'Database config:', { dbHost: dbConfig.dbHost, dbPort: dbConfig.dbPort, dbName: dbConfig.dbName });
    delete _databases[type][dbConfig.dbName];
    next(err);
  });
}
...
function _handleSlaveDB(type, tenantId, next) {
  ...
  app.tenantmanager.retrieveTenant({ _id: tenantId }, function (err, tenant) {
    ...
    if (!tenant || !tenant.database || !tenant.database.dbName) {
      return next(new Error('Tenant database configuration is missing.'));
    }
    const dbName = tenant.database.dbName;
    const dbInstance = _databases[type][dbName];
...
function checkConnection(cb) {
  ...
  getDatabase(function(error, db) {
    ...
    const checkReady = () => {
      ...
      if(db.conn.readyState === 0 || db.conn.readyState === 3) {
      ...
      // 2 = connecting, wait a bit more
      setTimeout(checkReady, 500);
    };
    // Give it up to 10 seconds to connect
    setTimeout(() => {
      if(db.conn.readyState !== 1) {
        return cb(`Cannot connect to the MongoDB '${dbName}' at '${dbHost}:${dbPort}'. Connection timed out. Please check the details are correct and the database is running.`);
      }
    }, 10000);
    checkReady();
  }, dbName);
...
function preloadHandle(app, instance) {
  return {
    ...
    moduleLoaded: function(modloaded) {
      ...
      // Ensure we have sensible defaults even if configuration hasn't populated yet
      if (!configuration.getConfig('dbName')) {
        configuration.setConfig('dbName', 'adapt-tenant-master');
      }
      if (!configuration.getConfig('dbHost')) {
        configuration.setConfig('dbHost', '127.0.0.1');
      }
      if (!configuration.getConfig('dbPort')) {
        // Default to standard MongoDB port for Electron portable app
        configuration.setConfig('dbPort', 27017);
      }
      if (!configuration.getConfig('dbType')) {
        configuration.setConfig('dbType', 'sqlite');
      }
      instance.getDatabase(function(err, db) {
        if (err) {
          // Debug output to console for development diagnosis
          try {
            // eslint-disable-next-line no-console
            console.error('[dev] failed to set app.db', {
          ...
          // Add context to understand why app.db setup failed
          return logger.log('error', 'failed to set app.db', {
            error: err && err.message ? err.message : err,
            tenantId: configuration.getConfig('dbName'),
            dbName: configuration.getConfig('dbName'),
            dbHost: configuration.getConfig('dbHost'),
...
function closeConnections(cb) {
  const tasks = [];
  Object.keys(_databases).forEach(type => {
    Object.keys(_databases[type]).forEach(dbName => {
      const dbInstance = _databases[type][dbName];
      if (dbInstance && typeof dbInstance.disconnect === 'function') {
        tasks.push(callback => {
          try {
            dbInstance.disconnect(callback);
          } catch (e) {
            callback();
          }
        });
